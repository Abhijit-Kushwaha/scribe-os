...
def find_credentials(packet):
    ...
    if packet.haslayer(TCP) and packet.haslayer(Raw):
        ...
        # Using regex for better pattern matching of credentials
        # This looks for common key-value pairs like user=...&pass=...
        credential_patterns = [
            r'(user(?:name)?|login|email|id|account)[\s=:]+([^\s&\'"]+)',
            r'(pass(?:word)?|pwd|secret)[\s=:]+([^\s&\'"]+)'
        ]
        for pattern in credential_patterns:
            if re.search(pattern, payload, re.IGNORECASE):
                print(f"[*] POTENTIAL CREDENTIAL FOUND: {payload[:200]}") # Print a snippet
                sys.stdout.flush()
                break # Found one, no need to check other patterns for this packet
...
def simulate_sniffing(capture_duration=60):
    ...
    while time.time() - start_time < capture_duration: # Run for specified duration
...
def start_sniffing(interface=None, capture_duration=60):
    ...
    if SCAPY_AVAILABLE:
        try:
            ...
            sys.stdout.flush()
            simulate_sniffing(capture_duration)
    else:
        simulate_sniffing(capture_duration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--duration", help="Duration of the capture in seconds", type=int, default=60)
    args = parser.parse_args()
    
    start_sniffing(interface=args.interface, capture_duration=args.duration)
