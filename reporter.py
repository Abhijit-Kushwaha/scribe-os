...
def generate_html_report():
    ...
    for row in rows:
        # Escape data to prevent HTML injection if database content is malicious
        target = html.escape(str(row[1]))
        status = html.escape(str(row[2]))
        risk = html.escape(str(row[3]))
        ts = html.escape(str(row[4]))
