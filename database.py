import os
from datetime import datetime
from supabase import create_client

# Supabase Configuration
# It's best practice to use environment variables for sensitive data like API keys.
URL = os.environ.get("SUPABASE_URL", "https://kewbyppxdgxkwtelcxed.supabase.co")
KEY = os.environ.get("SUPABASE_SERVICE_KEY") # Use a service role key for admin operations

if not KEY:
    print("[-] Supabase Error: SUPABASE_SERVICE_KEY environment variable not set.")
    # Exit or handle gracefully if the key is missing
    supabase = None
elif KEY.startswith("ey") and "sb_publishable" in KEY:
    print("[-] Supabase Error: You are using a public anon key. Please use a service role key for database operations.")
    supabase = None
else:
    supabase = create_client(URL, KEY)

def init_db():
    # Cloud mein table pehle se bani honi chahiye (SQL Editor use karein)
    print("[!] Cloud Database (Supabase) Connection Initialized.")

def save_loot(target, status, sqli_risk):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # Data structure jo Supabase table mein jayega
    data = {
        "target": target,
        "status": status,
        "sqli_risk": sqli_risk,
        "timestamp": timestamp
    }
    
    if not supabase:
        print("[-] Supabase client not initialized. Cannot save loot.")
        return

    try:
        # 'loot' table mein data insert karna
        supabase.table("loot").insert(data).execute()
        print(f"DATABASE_SAVED_CLOUD:{target}|RISK:{sqli_risk}")
    except Exception as e:
        print(f"[-] Supabase Error: {e}")

def self_destruct():
    # Cloud data delete karne ke liye
    if not supabase:
        print("[-] Supabase client not initialized. Cannot self-destruct.")
        return

    try:
        supabase.table("loot").delete().neq("target", "null").execute()
        print("SYSTEM_CLEANED: Cloud loot evidence destroyed.")
    except Exception as e:
        print(f"[-] Delete Error: {e}")

if __name__ == "__main__":
    init_db()